=== Sendsmaily ===
Tags: widget, plugin, sidebar, api, mail, email, marketing, sendsmaily
Tested up to: 5.2.2
Stable tag: 2.0.0
License: GPLv2 or later

Sendsmaily newsletter subscription plugin for WordPress

== Description ==

Sendsmaily newsletter subscription plugin for WordPress.

== Installation ==

* Install as regular plugin.
* Open smaily plugin settings from admin side panel menu "Smaily".
* Validate your credentials.
* Go to Appearance -> Widgets.
* Place "Sendsmaily newsletter subscription" widget where you wish.
* Using advanced tab in smaily settings will render advanced form.

== Changelog ==

= 2.0.0 =
* Changed DB schema and key column name to credentials. Implemented dbDelta functionality.

= 1.2.4 =
* Added support for an uninstall hook via uninstall.php, because two tables were being leftover after uninstall.

= 1.2.3 =
* Bugfix: Removes error message on plugin activation.

= 1.2.2 =
* Maintenance: Unifies table creation on installation.

= 1.2.1 =
* Advanced form changed from ajax to php handling to support Icegram free version.

= 1.2.0 =
* Changes because of new Smaily automation workflow

= 1.1.5 =
* Fixes SQL injection vector when caching autoresponders

= 1.1.4 =
* Tested compatibility with Wordpress 5.1

= 1.1.3 =
* Fixed advanced form display errors


= 1.1.2 =
* Renamed admin page to Smaily

= 1.1.1 =
* Fixed missing admin page icon.

= 1.1.0 =
* Fix warnings caused by trying to assign array variables to string.
* Rename Sendsmaily to Smaily.
* Use ajax for form submit.

= 1.0.0 =
* Coding standard fixes.
* Update to work with latest version of WP(4.4.2).
* Improved some of the structure.
* More smaller improvements.

= 0.9.1 =
* Fixed bug with URLs not working on Windows servers

= 0.9 =
* Public release
