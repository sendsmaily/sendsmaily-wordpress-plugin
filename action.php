...
$action = isset($_POST['op']) ? trim($_POST['op']) : '';
if(empty($action)){ die('No action or API key set.'); }
...
switch($action){
	case 'validateApiKey':
		...
		$request = new Wp_Sendsmaily_Request('https://www.sendsmaily.net/validate_key.php', array(
			'key' => $key
		));
		$result = $request->exec();
		$data = $result['data'];
		...
		$wpdb->query(
			sprintf('insert into `%s`(`key`,`domain`) values("%s","%s")', $tableName, $key, $data['domain'])
		);
		...
		$wpdb->query(
			sprintf('insert into `%s`(`id`,`title`) values%s', $tableName, implode(',', $insertQuery))
		);
	...
	case 'save':
		...
		$wpdb->query(
			sprintf('update `%s` set autoresponder="%s", success_url="%s", failure_url="%s", form="%s", is_advanced="%s"',
				$tableName, $basic['autoresponder'], $basic['success_url'], $basic['failure_url'], addslashes($advanced['form']), $isAdvanced
			)
		);
